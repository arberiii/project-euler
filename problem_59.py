...
def encode(letters):
    return "".join([chr(int(letter)) for letter in letters])
...
def decrypt_by_key(letters, key):
    x = ""
    for ind in range(len(letters)):
        x = x + xor(chr(letters[ind]), chr(key[ind % 3]))
    return x


# Solution to find the key

for i in range(ord('a'), ord('z') + 1):
    for j in range(ord('a'), ord('z') + 1):
        for k in range(ord('a'), ord('z') + 1):
            possible_key = [i, j, k]
            word = decrypt_by_key([int(x) for x in encrypted_letters], possible_key)
            if "the" in word and "and" in word and "is" in word and "of" in word:
                print(word)
                print(possible_key)
...
word = decrypt_by_key([int(x) for x in encrypted_letters], KEY)
s = 0
